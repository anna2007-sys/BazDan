...
--1
SELECT category, COUNT(p.product_id) FROM Products p GROUP BY p.category;
...
--4
SELECT AVG(order_total) FROM ( SELECT order_id, SUM(quantity * price_per_unit) AS order_total FROM Order_Items GROUP BY order_id);

--5
SELECT status, COUNT(o.status) FROM Orders o GROUP BY o.status;

--6
SELECT category FROM Products p GROUP BY p.category HAVING COUNT(*) > 1;

--7
SELECT c.full_name FROM Customers c JOIN Orders o ON c.customer_id = o.customer_id GROUP BY c.customer_id, c.full_name HAVING COUNT(o.order_id) > 1;
